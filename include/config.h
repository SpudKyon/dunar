...
#ifndef DUNAR_CONFIG_H
#define DUNAR_CONFIG_H
 ...
 public:
  ...
  static typename ConfigVar<T>::ptr Lookup(
    ...
    if (it != GetDatas().end()) {
      ...
      if (tmp) {
        DUNAR_LOG_INFO(DUNAR_LOG_ROOT()) << "Lookup name=" << name << " exists";
        return tmp;
      } else {
        DUNAR_LOG_ERROR(DUNAR_LOG_ROOT())
    ...
    if (name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678") !=
        std::string::npos) {
      DUNAR_LOG_ERROR(DUNAR_LOG_ROOT()) << "Lookup name invalid " << name;
