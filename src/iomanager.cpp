...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
  ...
  if (fd_ctx->events & event) {
    ...
    DUNAR_ASSERT(!(fd_ctx->events & event));
  }
  ...
  DUNAR_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
  ...
  if (cb) {
    ...
    DUNAR_ASSERT(event_ctx.fiber->getState() == Fiber::EXEC);
  }
  return 0;
}
