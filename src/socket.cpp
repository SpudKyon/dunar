...
Socket::ptr Socket::CreateTCP(dunar::Address::ptr address) {
  Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
  return sock;
}

Socket::ptr Socket::CreateUDP(dunar::Address::ptr address) {
...
Socket::ptr Socket::accept() {
  ...
  if (newsock == -1) {
    DUNAR_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno
                              << " errstr=" << strerror(errno);
    return nullptr;
  }
...
bool Socket::bind(const Address::ptr addr) {
  if (!isValid()) {
    newSock();
    if (DUNAR_UNLICKLY(!isValid())) {
      return false;
    }
  }

  if (DUNAR_UNLICKLY(addr->getFamily() != m_family)) {
...
bool Socket::listen(int backlog) {
  if (!isValid()) {
    DUNAR_LOG_ERROR(g_logger) << "listen error sock=-1";
    return false;
  }
...
bool Socket::cancelRead() {
  return IOManager::GetThis()->cancelEvent(m_sock, dunar::IOManager::READ);
}

bool Socket::cancelWrite() {
  return IOManager::GetThis()->cancelEvent(m_sock, dunar::IOManager::WRITE);
}
