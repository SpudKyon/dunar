...
bool Address::getInterfaceAddresses(
  ...
  if (getifaddrs(&results) != 0) {
    DUNAR_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                 " err="
  ...
  try {
    ...
    DUNAR_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
    freeifaddrs(results);
    return false;
  }
  freeifaddrs(results);
...
IPAddress::ptr IPAddress::create(const char* address, uint16_t port) {
  ...
  if (error) {
    DUNAR_LOG_ERROR(g_logger) << "IPAddress::create(" << address << ", " << port
...
IPv4Address::ptr IPv4Address::create(const char* address, uint16_t port) {
  ...
  if (result <= 0) {
    DUNAR_LOG_ERROR(g_logger) << "IPv4Address::create(" << address << ", "
...
IPv6Address::ptr IPv6Address::create(const char* address, uint16_t port) {
  ...
  if (result <= 0) {
    DUNAR_LOG_ERROR(g_logger) << "IPv6Address::create(" << address << ", "
                              << port << ") rt=" << result << " errno=" << errno
                              << " errstr=" << strerror(errno);
    return nullptr;
  }
