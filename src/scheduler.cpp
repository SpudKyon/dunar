...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string& name)
    : m_name(name) {
  DUNAR_ASSERT(threads > 0);

  if (use_caller) {
    Fiber::GetThis();
    --threads;

    DUNAR_ASSERT(GetThis() == nullptr);
    t_scheduler = this;
...
void Scheduler::stop() {
  ...
  if (m_rootFiber && m_threadCount == 0 &&
    ...
    DUNAR_LOG_INFO(g_logger) << this << " stopped";
    m_stopping = true;
...
void Scheduler::tickle() { DUNAR_LOG_INFO(g_logger) << "tickle"; }
