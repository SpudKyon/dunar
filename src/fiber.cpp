...
Fiber::Fiber() : m_ctx() {
  ...
  SetThis(this);
  if (getcontext(&m_ctx)) {
    DUNAR_ASSERT2(false, "getcontext");
  }
  ++s_fiber_count;
  DUNAR_LOG_DEBUG(g_logger) << "Fiber::Fiber";
}
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool use_caller)
  ...
  DUNAR_LOG_DEBUG(g_logger) << "Fiber::Fiber id=" << m_id;
}

Fiber::~Fiber() {
  --s_fiber_count;
  if (m_stack) {
    DUNAR_ASSERT(m_state == TERM || m_state == EXCEPT || m_state == INIT);
    StackAllocator::Dealloc(m_stack, m_stacksize);
  } else {
    DUNAR_ASSERT(!m_cb);
    DUNAR_ASSERT(m_state == EXEC);
    Fiber* cur = t_fiber;
    if (cur == this) {
      SetThis(nullptr);
    }
...
void Fiber::call() {
  SetThis(this);
  m_state = EXEC;
  DUNAR_LOG_ERROR(g_logger) << getId();
  if (swapcontext(&t_threadFiber->m_ctx, &m_ctx)) {
    DUNAR_ASSERT2(false, "swapcontext");
  }
}
...
Fiber::ptr Fiber::GetThis() {
  ...
  DUNAR_ASSERT(t_fiber == main_fiber.get());
...
void Fiber::MainFunc() {
  ...
  try {
    ...
    cur->m_state = EXCEPT;
    DUNAR_LOG_ERROR(g_logger) << "Fiber Except: " << ex.what()
                              << " fiber_id=" << cur->getId() << std::endl
                              << dunar::BacktraceToString();
  } catch (...) {
    cur->m_state = EXCEPT;
    DUNAR_LOG_ERROR(g_logger) << "Fiber Except"
                              << " fiber_id=" << cur->getId() << std::endl
                              << dunar::BacktraceToString();
  }
