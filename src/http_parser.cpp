...
static dunar::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    dunar::Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024),
                          "http request buffer size");

static dunar::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    dunar::Config::Lookup("http.request.max_body_size",
...
void on_request_http_field(void *data, const char *field, size_t flen,
  ...
  HttpRequestParser *parser = static_cast<HttpRequestParser *>(data);
  if (flen == 0) {
    DUNAR_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
HttpRequestParser::HttpRequestParser() : m_error(0) {
  m_data.reset(new dunar::http::HttpRequest);
...
void on_response_http_field(void *data, const char *field, size_t flen,
  ...
  HttpResponseParser *parser = static_cast<HttpResponseParser *>(data);
  if (flen == 0) {
    DUNAR_LOG_WARN(g_logger) << "invalid http response field length == 0";
    parser->setError(1002);
    return;
  }
  parser->getData()->setHeader(std::string(field, flen),
...
HttpResponseParser::HttpResponseParser() : m_error(0) {
  m_data.reset(new dunar::http::HttpResponse);
